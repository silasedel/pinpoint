...
- **Scoring**: `5000 * e^(-distance_km / 1492.7)`. Solo scores live in localStorage, one board per mode: the home page shows Classic only (with the rounds and time filters), and every other mode gets its own leaderboard page, reached from **View leaderboard** at the end of a game or from **Every mode's leaderboard** under the home board. The page has the mode's own rounds/time filters, its last 25 scores with dates, a per-mode clear, and a picker to jump straight to any other mode's board. Entries saved before modes existed are matched back to their mode by their label.
