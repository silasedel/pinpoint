...
**Editing a map.** Open a map you made and there is an Edit map button under Play. It reopens the builder with every spot loaded (place names fill in as you watch), and Save changes republishes under the same id, so the link, plays, likes, comments and board all survive. In the builder, before or after publishing, each row has a Look button (and the thumbnail is clickable) that opens the spot full screen in Pinpoint's own viewer, drawn from the Street View tiles: drag to turn and tilt, scroll or + − to zoom, Previous, Next, Remove this spot and an X, so you can walk the whole map and throw out anything that landed in the wrong place. The next spot's tiles load in the background as you go. A Google view button swaps in Google's embed for walking around the spot (it's read-only to the page, which is why the viewer is ours). Review spots starts that from the top. Set facing keeps whatever the viewer is showing, direction and tilt, as the spot's start view (the row notes "faces 217°", and rooms get it too). Check for missing Street View asks Google about every spot, flags the ones it no longer has, and a Remove missing spots button clears them in one go.
