...
- **Capitals**: Country mode, but every drop is inside a capital city (~100 capitals with official coverage, within about 2 km of the center). Click the country.
...
- **Deported**: every round drops you in a country that borders the last one, using a land-border graph of the countries the game can reach. A failed lookup can't break the chain: the step is retried, then each neighbor is tried on its own with a wider snap, and only if nothing at all is reachable does it start a new chain — which the result screen says out loud. The result screen also shows the trail so far (Italy → Slovenia → **Austria**) and whether the next round really is a neighbor.
...
- **Learn**: its own bar on the home page. A normal game, your choice of time and length, on a set of hand-picked spots. Each spot was looked at before it was written up, and the round ends with the two or three things in that exact view that give the place away and where else you'd see them (steep metal roofs and carports mean snow country, so Hokkaido or the Sea of Japan coast; red asphalt bike paths with the speed limit painted on the road mean the Netherlands). Seventy-two spots across fifty-odd countries; ones you haven't seen come first, so nothing repeats until you've been through them all.
- **Lives** (multiplayer only): the same, with three lives each. The farthest guess loses one; you're out at zero. The round counter in the corner becomes your three hearts. Losing one makes that heart swell, shake and drain to gray; losing your last one cracks it down the middle with the split drawing itself in. The reveal banner shows the same hearts breaking and plays a short falling tone for a life lost, a heavier one for being knocked out. Round rankings and final standings show hearts too, with a cracked one for anybody out. The round count is sized to the table so the game can finish. In Lives and Battle Royale the round recap shows no points at all: the top line says how many are still standing and your own hearts, and each row is a name, this round's distance and that player's hearts (or in / out), sorted by who has the most left.
...
Anyone signed in can build a map: give it a name, click spots on a world map (each click snaps to the nearest Street View within a couple of miles and shows a thumbnail), or type an address, a place name, coordinates or a Google Maps link into the box to add one exactly (addresses and place names are looked up with OpenStreetMap's Nominatim, with Photon as a fallback; a Google Maps place link is read for its name or coordinates, though goo.gl short links can't be followed from a browser). Ten spots minimum (a progress bar counts you up to it), two thousand maximum (past 400 the place names are left out of the record to keep it light), and you need an account to make one; the Create a custom map bar on the home page takes you straight to the builder. Publishing makes it one retained message (`worlddrop/v1/cmap/<id>`) with a shareable link (`?m=<id>`). The **Custom maps** bar under the mode grid — on the solo setup screen and in a lobby — lists every map on the brokers with a search box, sorted by plays. Playing one is Classic scoring over a shuffle of its spots, with your choice of time and length; each map keeps its own small board of best scores rather than touching the mode boards, and the maker can delete it.

**Leaving the builder.** Back asks first when there is unsaved work: a new map with spots that hasn't been published, or an edited map whose changes haven't been saved. Cancel keeps you there; Leave anyway throws the work away. Closing or reloading the tab with unsaved spots gets the browser's own warning.

**Editing a map.** Open a map you made and there is an Edit map button under Play. It reopens the builder with every spot loaded (place names fill in as you watch), and Save changes republishes under the same id, so the link, plays, likes, comments and board all survive. In the builder, before or after publishing, each row has a Look button (and the thumbnail is clickable) that opens the spot full screen in Pinpoint's own viewer, drawn from the Street View tiles at exactly the width a player gets (110°, calibrated against Google's widest view on the same panorama), filling the screen with no bars. Drag to turn and tilt, scroll to zoom, and walk with the round arrows, which are the same ones Pixelated uses: one sits where each neighboring road leaves the picture, or at the edge of the screen turned toward a road behind you. Walking moves the spot to that position, so you can step down the street to exactly the place you want. Remove drops the spot.

**Checking spots one by one.** Check spots one by one walks the map in the order the spots were added. Lock Position (or Enter, or the right arrow) keeps whatever the viewer is showing, position, direction and tilt, as that spot's start view and moves to the following spot; Remove spot sits just to its left. Nothing skips and nothing cycles; Previous steps back. How far you got is remembered per map on this device, and each locked spot is flagged in the map record itself, so on any device the button reads "Continue checking (312 of 1609 done)" next time and the checked rows carry a tick. Maps locked before the flag existed resume at the end of the run of spots that carry a tilt. After the last spot the viewer closes and the button becomes Check spots again. Older maps' spots that were never checked start facing the way they were saved. Rooms get the saved facing too.

**Builder tools.** Check for missing Street View asks Google about every spot, flags the ones it no longer has, and a Remove missing spots button clears them in one go. Remove near-duplicates finds spots within 25 m of an earlier spot (two clicks on one corner, a walk that landed beside another spot) and drops the later ones after asking. In the checker, Remove spot always lands on the spot that followed and says so; when that one is within 40 m of the removed one the note calls it a near-duplicate, because the list renumbers and the two can look identical.

**Getting spots exactly right.** A typed address or coordinates is looked up, then Street View is searched in widening rings (40 m, 120 m, 300 m, 800 m, 3.5 km) so the spot lands on the street you asked for rather than the nearest covered road a block over, and the view is turned to face the point you typed. Typed spots just join the list (open one with Look if you want to see it), and the builder says how far from the point the nearest Street View was if it's more than 60 m. Map clicks search 250 m, then 1.2 km, then 3.5 km.
...
**The map's board, likes and comments.** Finishing a game on a custom map puts that game on the map's board (every finished game, the best fifty kept, 25 shown), and the final screen says where you landed with a View map board button. Anyone signed in can like a map (a heart on the map page, the count on every card) and comment on it; comments live in their own retained message (`worlddrop/v1/cmc/<id>`, newest first, 150 kept), the maker gets a bell notification with an Open button, and a comment can be deleted by its author, the maker or admin.

**Map analytics.** Settings has a Map analytics button. Every finished play on a custom map adds one play and its seconds to that day in the map's own record (`stats`, keyed by date), so the maker gets a YouTube-style dashboard: a headline for the period, tiles for plays, play time in hours and average play duration with the change against the previous period, likes across your maps, a line chart of the chosen tile per day, a range dropdown (last 7 days, last 28 days, lifetime) and the maps ranked for the period with their thumbnail, average play duration, likes and plays. The tiles are plays, play time in hours and likes (likes are counted per day from v3.4.5 on, so the chart and the previous-period change work for them too); average play duration lives in the per-map list only. Map analytics only appears in Settings once you have a map. Plays are counted when a game finishes, on a freshly fetched copy of the record, so two players finishing at once rarely overwrite each other.

## Challenges

Any finished solo or party game (not the daily, not the crowd-only modes) has a **Share** button in the bottom-left corner of the end screen. It opens a small card with one sentence and a link. Whoever opens the link sees the challenge page — who set it, the mode, their score, who else has played — and plays the exact same rounds. After every guess the sharer's pin appears on the result map in their color with a dashed line, alongside a running comparison, like racing a ghost. The end screen says who won, and the sharer gets a notification (*so-and-so completed your challenge · 19,800 vs your 21,400*) whose **See** button opens the page with a round-by-round table. A challenge is one retained message (`worlddrop/v1/ch/<id>`) holding the locations, the sharer's guesses and everyone's plays; one attempt per person.
...
3. Everyone picks a name and a color (no two players can share a color). If you have a profile photo, a thirteenth tile at the front of the tray is your photo, already selected — it becomes your token in the player list, the strip, the standings and the head of your pin. The host picks time, rounds and a mode, and presses Start. The game begins immediately.
...
5. Every round shows the same panorama to everyone. When all guesses are in, or time runs out, the map shows every pin in its player's color with distances, points, and running totals. The host advances rounds. 2 to 10 players. The final map shows every round's real spot and every player's guess for it, each line in that player's color — for a photo token, the color that stands out most in the photo.
...
**Loop games.** A switch under Rounds, for a manager who is hosting. With it on the server never stops: the final screen stays up for five seconds and the next game starts straight from it, no lobby in between, with the same settings, even if you are the only one in it. Round recaps move on by themselves after ten seconds too, so nobody has to press Next. If the host leaves or closes the tab, whoever takes over starts the next game on arrival, and anyone, not just a manager, can wake a looping server that fell asleep; it starts a game as soon as they are in. Only a manager hosting it sees Back to lobby on the final screen, which is how you pause it to change settings.
...
- **Sound** is synthesised in the browser with WebAudio, so there are no audio files. The interaction sounds are mechanical, not musical: a snap with a small thump when you place a pin, a click for buttons and chips, a click and a heavier latch when you lock in, a quiet tick as the clock runs down. The bigger moments stay tonal through one soft low-pass bus: a low pad on the reveal, a single quiet tone that glides up while the score counts, a chord for a great guess and a fuller one with a high shimmer for a near-perfect. The speaker button in the bottom-left corner mutes it and the choice is remembered.
...
- **Pins.** Every guess is a white pin with the player's color — or photo — in its head. The real location is a bigger red pin with a target in it and a pulsing ring at its foot, so it's never lost among the guesses.
