...
**Editing a map.** Open a map you made and there is an Edit map button under Play. It reopens the builder with every spot loaded (place names fill in as you watch), and Save changes republishes under the same id, so the link, plays, likes, comments and board all survive. In the builder, before or after publishing, each row has a Look button (and the thumbnail is clickable) that opens that exact Street View full screen with Previous, Next, Remove this spot and an X, so you can walk the whole map and throw out anything that landed in the wrong place. Review spots starts that from the top. Set facing in the preview swaps Google's embed for the draggable viewer, full screen: turn and tilt until you are looking the way players should start, zoom if you like, press Face this way, and that heading and tilt become the spot's start view (the row notes "faces 221°", and rooms get it too). Check for missing Street View asks Google about every spot, flags the ones it no longer has, and a Remove missing spots button clears them in one go.
