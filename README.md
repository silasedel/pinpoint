...
**Editing a map.** Open a map you made and there is an Edit map button under Play. It reopens the builder with every spot loaded (place names fill in as you watch), and Save changes republishes under the same id, so the link, plays and board all survive. In the builder, before or after publishing, each row has a Look button (and the thumbnail is clickable) that opens that exact Street View full screen with Previous, Next and Remove this spot, so you can walk the whole map and throw out anything that landed in the wrong place. Review spots starts that from the top. Check for missing Street View asks Google about every spot, flags the ones it no longer has, and a Remove missing spots button clears them in one go.

**The map's board.** Finishing a game on a custom map puts you on that map's board (one row per account, your best score), and the final screen says where you landed with a View map board button. Signed out, it tells you to sign in to get on it.
