...
**Editing a map.** Open a map you made and there is an Edit map button under Play. It reopens the builder with every spot loaded (place names fill in as you watch), and Save changes republishes under the same id, so the link, plays, likes, comments and board all survive. In the builder, before or after publishing, each row has a Look button (and the thumbnail is clickable) that opens the spot full screen in Pinpoint's own viewer, drawn from the Street View tiles at the same width a player gets: drag to turn and tilt, scroll or + − to zoom, and walk with the arrows (an arrow sits where each neighboring road leaves the picture, or is pinned to the side of the screen when that road is behind you). Walking moves the spot to that position, so you can step down the street to exactly the place you want. Set facing keeps whatever the viewer is showing, direction and tilt, as the spot's start view (the row notes "faces 217°", and rooms get it too). Remove drops the spot.

**Checking every spot once.** Every spot you open in the viewer is remembered as checked, per map, on your device (it survives publishing, editing and reloads). Review spots opens the first unchecked one, Next (or the right arrow, or Enter) always goes to the next unchecked one, the bar says how many are left, checked rows get a tick in the list, and when none are left Next says so and the button turns into Review again, which clears the marks. Previous still walks backward through everything.
