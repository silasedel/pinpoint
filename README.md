...
Admin belongs to one account. It needs two things: being signed in as that handle, and the admin phrase entered once on the device (only its SHA-256 lives in the page, in `ADMIN_HASH`, and the device keeps the hash in `localStorage`, never the phrase). The card only appears in Settings for that account; nobody else sees an admin control anywhere. The handle can't be claimed fresh (a broker outage used to make that possible), and rewriting the account record on a broker gains nothing without the phrase. Passcodes can now be 4 to 12 digits, so the admin account can carry a long one.

While admin is on, every leaderboard row (mode boards, the daily, each custom map's board) has a pencil that renames that row and a trash button that removes it, every custom map has Edit map and Delete, and the map list has a trash on each card. The Settings card can rename an account (the record plus every board row carrying its handle), remove an account, or ban it. A ban puts the handle on a retained list (`worlddrop/v1/ban/handles`) that every client honors: the handle can't be claimed again, its rows are dropped from every board and from the local mirror that would otherwise re-upload them, its maps are deleted, and a browser still signed in as it is signed out. Removed score ids keep their own list (`worlddrop/v1/ban/ids`). Hide controls this visit tucks the buttons away while playing without forgetting the phrase.
